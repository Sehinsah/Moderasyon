...
client.on("message", message => {
    ...
    return message.channel.send(`<a:sonsuzkalp:793345179376680971> ${message.author}, Aleyküm Selam.`)
});
...
db.delete(`cezali_${member.guild.id + member.id}`)
    ...
    member.roles.remove('800227270412337162');
  }, ms(sürejail));
}
})
...
db.delete(`muteli_${member.guild.id + member.id}`)
    ...
    member.roles.remove('800225974234120202');
  }, ms(süre));
}
})
